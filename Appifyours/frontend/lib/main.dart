...
class CartManager extends ChangeNotifier {
  ...
  String get displayCurrencySymbol => _displayCurrencySymbol;
  ...
  double get gstAmount {
    return (subtotal - totalDiscount) * 0.18;
  }
...
class ApiConfig {
  ...
  static String get baseUrl => 'http://192.168.0.6:5000';
}
...
class ChatBotPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Chat with $shopName'),
      ),
      body: const Center(
        child: Text('Chat Bot Page - Coming Soon'),
      ),
    );
  }
}

// Delivery Checkout Page (placeholder)
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text('Total: ${PriceUtils.formatPrice(cartManager.finalTotal)}'),
            ElevatedButton(
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
...
class _HomePageState extends State<HomePage> {
  ...
  final MobileSessionManager _mobileSessionManager = MobileSessionManager();
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (responseJson['success'] == true && responseJson['presignedUrl'] != null) {
            ...
            _mobileSessionManager.profileImage = newUrl;
          }
        } catch (e) {
  ...
  Widget _buildProductCard(Map<String, dynamic> product, int index, {Map<String, dynamic>? styleProps}) {
    ...
    final String discountLabel = hasPercentDiscount ? '${badgeDiscountPercent.round()}% OFF' : 'OFFER';
    final String stockLabel = isSoldOut ? 'SOLD OUT' : 'In stock: $quantityAvailable';
                    ...
                    child: Stack(
                      children: [
                        CircleAvatar(
                          ...
                          backgroundImage: _mobileSessionManager.profileImage != null && _mobileSessionManager.profileImage!.isNotEmpty
                              ? NetworkImage(_mobileSessionManager.profileImage!)
                              : null,
                          child: _isUploadingProfileImage
                              ? const CircularProgressIndicator(color: Colors.white)
                              : (_mobileSessionManager.profileImage == null || _mobileSessionManager.profileImage!.isEmpty)
